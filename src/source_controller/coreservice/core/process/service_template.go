...
func (p *processOperation) CreateServiceTemplate(kit *rest.Kit, template metadata.ServiceTemplate) (
	*metadata.ServiceTemplate, errors.CCErrorCoder) {
	...
	if err != nil {
		blog.Errorf("category id invalid, code: %d, err: %v, rid: %s", common.CCErrCommParamsInvalid, err, kit.Rid)
	...
	if ccErr != nil {
		blog.Errorf("check leaf node failed, err: %v, rid: %s", ccErr, kit.Rid)
		return nil, ccErr
	}
	if !isLeafNode {
	...
	if bizID != category.BizID && category.BizID != 0 {
		blog.Errorf("input bizID %d not equal category bizID %d, rid: %s", bizID, category.BizID, kit.Rid)
		return nil, kit.CCError.CCErrorf(common.CCErrCommParamsInvalid, common.BKAppIDField)
	}
...
func (p *processOperation) ListServiceTemplates(kit *rest.Kit, option metadata.ListServiceTemplateOption) (
	*metadata.MultipleServiceTemplate, errors.CCErrorCoder) {
	...
	if option.ServiceCategoryID != nil && *option.ServiceCategoryID > 0 {
		...
		if err != nil {
			blog.Errorf("ListServiceCategories failed, err: %v, rid: %s", err, kit.Rid)
			return nil, err
		}
...
func (p *processOperation) DeleteServiceTemplate(kit *rest.Kit, serviceTemplateID int64) errors.CCErrorCoder {
	...
	if err != nil {
		blog.Errorf("GetServiceTemplate failed, templateID: %d, err: %v, rid: %s", serviceTemplateID, err, kit.Rid)
		return err
	}
	...
	if usageCount > 0 {
		blog.Errorf("forbidden delete service template be referenced, code: %d, rid: %s",
