...
func (c *Client) getEventDetailFromMongo(kit *rest.Kit, node *watch.ChainNode, fields []string, key event.Key) (
	*string, bool, error) {
	...
	if key.Collection() == common.BKTableNameBaseInst || key.Collection() == common.BKTableNameMainlineInstance {
		...
		collection, err = object.GetInstTableNameByObjID(kit, node.SubResource[0])
		if err != nil {
			return nil, false, err
		}
	}
...
func (c *Client) searchFollowingEventChainNodes(kit *rest.Kit, opts *searchFollowingChainNodesOption) (
	...
	if err != nil {
		...
		err := c.watchDB.Shard(kit.ShardOpts()).Table(common.BKTableNameLastWatchEvent).Find(filter).
...
func (c *Client) searchEventDetailsFromMongo(kit *rest.Kit, nodes []*watch.ChainNode, fields []string,
	...
	coll := key.Collection()
	objInstIDsMap := make(map[string][]int64)
	delOidDetailMap := make(map[int]string)
	for _, node := range nodes {
		if node.EventType == watch.Delete {
			// delete event detail can not be retrieved from mongo
			delOidDetailMap[errCursorIndexMap[node.Cursor]] = "{}"
			continue
		}
		objectId, err := primitive.ObjectIDFromHex(node.Oid)
		if err != nil {
		...
		if coll == common.BKTableNameBaseInst || coll == common.BKTableNameMainlineInstance {
			...
			objInstIDsMap[node.SubResource[0]] = append(objInstIDsMap[node.SubResource[0]], node.InstanceID)
		}
	...
	if len(oids) > 0 {
		var err error
		oidDetailMap, err = c.getDetailsByOids(kit, oids, fields, coll, oidIndexMap, objInstIDsMap)
		if err != nil {
			return nil, err
		}
	}

	for idx, detail := range delOidDetailMap {
		oidDetailMap[idx] = detail
	}
...
func (c *Client) getDetailsByOids(kit *rest.Kit, oids []primitive.ObjectID, fields []string, coll string,
	oidIndexMap map[string][]int, objInstIDsMap map[string][]int64) (map[int]string, error) {
