...
var _ = Describe("host abnormal test", func() {
	Describe("test preparation", func() {
		...
		It("get instance topo", func() {
			rsp, err := instClient.GetInternalModule(context.Background(), strconv.FormatInt(bizId1, 10), header)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
			Expect(rsp.Result).To(Equal(true))
	...
	Describe("add host test", func() {
		...
		Describe("add host using api test", func() {
			...
			AfterEach(func() {
				...
				rsp, err := hostServerClient.SearchHostWithNoAuth(context.Background(), header, input)
				util.RegisterResponseWithRid(rsp, header)
				Expect(err).NotTo(HaveOccurred())
				Expect(rsp.Result).To(Equal(true))
	...
	Describe("search host test", func() {
		...
		It("search host using invalid bk_host_id", func() {
			rsp, err := hostServerClient.GetHostInstanceProperties(context.Background(), "eceer", header)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
			Expect(rsp.Result).To(Equal(false))
		})
		...
		It("search host using noexist bk_host_id", func() {
			rsp, err := hostServerClient.GetHostInstanceProperties(context.Background(), noExistIDStr, header)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
			Expect(rsp.Result).To(Equal(false))
		})
	...
	Describe("transfer host test", func() {
		...
		It("transfer multiple hosts with a nonexist host to module", func() {
			...
			rsp1, err := hostServerClient.SearchHostWithNoAuth(context.Background(), header, input1)
			util.RegisterResponseWithRid(rsp1, header)
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("batch operate host", func() {
		...
		It("update host one nonexist attr", func() {
			...
			rsp1, err := hostServerClient.GetHostInstanceProperties(context.Background(),
...
func prepareData() {
	...
	rsp3, err := hostServerClient.SearchHostWithNoAuth(context.Background(), header, input3)
	...
	rsp5, err := hostServerClient.SearchHostWithNoAuth(context.Background(), header, input5)
