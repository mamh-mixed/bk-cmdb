...
func Upsert(ctx context.Context, db dal.RDB, tableName string, row interface{}, idField string, uniqueKeys []string,
	ignores []string) (instID uint64, preData map[string]interface{}, err error) {
	...
	if idField != "" {
		id, err := util.GetInt64ByInterface(existOne[idField])
		if err != nil {
			return 0, nil, fmt.Errorf("reverse to int failed, err: %v", err)
		}
		instID = uint64(id)
		if instID <= 0 {
			instID, err = db.NextSequence(ctx, tableName)
...
func Insert(ctx context.Context, db dal.RDB, tableName string, row interface{}, idField string,
	uniqueKeys []string) error {
	if idField == "" {
