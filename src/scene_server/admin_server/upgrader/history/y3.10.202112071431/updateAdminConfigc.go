...
func updatePlatformConfigAdmin(ctx context.Context, db dal.RDB, conf *history.Config) error {
	...
	// 将上一份配置文件写入到history中
	history.AddConfigAdminChangeItem("y3.10.202105261459", "调整拓扑实例和模板名称的校验规则的配置", config)

	// 将本次平台管理配置文件写入history中
	history.AddConfigAdminChangeItem("y3.10.202112071431", "调整平台管理配置", InitAdminConfig)

	return history.UpgradePlatConfigAdmin(ctx, db, "y3.10.202112071431")
}
