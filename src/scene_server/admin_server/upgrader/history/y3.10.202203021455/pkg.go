...
func init() {
	history.RegistUpgrader("y3.10.202203021455", upgrade)
}

func upgrade(ctx context.Context, db dal.RDB, conf *history.Config) (err error) {
	blog.Infof("start execute y3.10.202203021455")

	if err = addUncategorizedClassification(ctx, db, conf); err != nil {
		blog.Errorf("upgrade y3.10.202203021455 add uncategorized classification failed, err: %v", err)
		return err
	}

	if err = changeClassificationType(ctx, db); err != nil {
